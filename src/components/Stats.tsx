        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-32">
          {stats.map((s, i) => (
            <motion.div 
              ...
              className="bg-zinc-900/50 border border-zinc-800 p-6 md:p-12 rounded-[1.5rem] md:rounded-[2.5rem] text-center group hover:border-neon transition-all duration-500 flex flex-col justify-center"
            >
              <div className="text-3xl md:text-6xl font-black text-neon mb-2 md:mb-3 group-hover:drop-shadow-[0_0_15px_rgba(223,255,0,0.5)] transition-all leading-none">{s.val}</div>
              <div className="text-zinc-500 text-[8px] md:text-xs font-black tracking-[0.1em] md:tracking-[0.2em] leading-tight">{s.label}</div>
            </motion.div>
          ))}
        </div>
